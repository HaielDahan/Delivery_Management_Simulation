...
public class PostSystemPanel extends JPanel implements ActionListener {
   ...
   public void createNewPostSystem() 
   {   
	   game = MainOffice.getInstance();
	   game.setPanel(this);
	   
	   packagesNumber = MainOffice.getNumOfPackages();
	   trucksNumber = MainOffice.getNumOfTrucksForBranches();
	   branchesNumber = MainOffice.getNumOfBranches();
   ...
   public void start() 
   {
	   if (game == null || this.started) return;
   ...
   {   
	   ...
	   String[] options = { "OK", "Cancel" };
	   String title = "Choose branch";
	   int selection = JOptionPane.showOptionDialog(null, cb, title, JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE, null, options, options[0]);

	   if (selection == 1) return;
	   
	   origin.setState((MainOffice)game.clone());
	   cTaker.add(origin.saveStateToMemento());
	   this.counterOfClone++;
	   
	   Branch cloneBranch = (Branch)game.getHub().getBranches().get(cb.getSelectedIndex()).clone();
	   game.getHub().add_branch(cloneBranch);
	   branchesNumber++;
   }
   ...
   public void restore()
   {
	   if (this.counterOfClone > 0)
	   {
		   game=(MainOffice)cTaker.get(--counterOfClone).getState().clone();
		   game.restore(game);
		   this.started = false;
		   ReadWriteFile rWf = new ReadWriteFile();
		   rWf.setNumOfLine();
		   rWf.writeToFile(null);
		   start();
	   }
   }
   ...
   public void report()
   {
	   String pathOfTxtFile = "tracking.txt";
	   File file = new File(pathOfTxtFile);
   ...
   public void actionPerformed(ActionEvent e) 
   {
		...
		{
			if (game == null) createNewPostSystem();
			start();
		}
