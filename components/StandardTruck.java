...
public class StandardTruck extends Truck{
	...
	public void load (Branch sender, Branch dest, Status status) {
		...
		synchronized(sender) {
			for (int i=0; i< sender.listPackages.size();i++) {
				...
				if (p.getStatus()==Status.BRANCH_STORAGE || (p.getStatus()==Status.HUB_STORAGE && MainOffice.getInstance().getHub().getBranches().get(p.getDestinationAddress().zip)==dest)) {
	...
	{
		Random r = new Random();
		StandardTruck cloneStandardTruck = new StandardTruck((r.nextInt(900)+100)+"-"+(r.nextInt(90)+10)+"-"+(r.nextInt(900)+100), this.getTruckModel(), this.maxWeight);
		
		return cloneStandardTruck;
	}	
}
