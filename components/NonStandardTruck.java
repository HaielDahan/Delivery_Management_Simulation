...
public class NonStandardTruck extends Truck{
			...
			if (!this.isAvailable()) {
				...
				if (this.getTimeLeft()==0) {
					if (p.getStatus()==Status.COLLECTION) {
						...
						p.addTracking(new Tracking(MainOffice.getClock(), null, p.getStatus(), p.getPackageID()));
						setAvailable(true);
					}
				}
			}
	...
	{
		Random r = new Random();
		NonStandardTruck cloneNonStandardTruck = new NonStandardTruck((r.nextInt(900)+100)+"-"+(r.nextInt(90)+10)+"-"+(r.nextInt(900)+100), this.getTruckModel(), this.length, this.width, this.height);
		
		return cloneNonStandardTruck;
	}	
}
