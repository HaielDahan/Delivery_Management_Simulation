...
public abstract class Package {
	...
	private String serialOfCustomer;
	private ReadWriteFile readWriteFile = new ReadWriteFile();
	
	public String getStringOfLastTracking()
	{
		if (this.tracking.isEmpty()) return null;
		else return "serial number of customer: " + this.serialOfCustomer + ", " + tracking.get(this.tracking.size() - 1).toString();
	}
...
//	{
//		if (this.tracking.isEmpty()) return null;
//		else return this.tracking.get(this.tracking.size() - 1);
//	}
	
	
	public Package(Priority priority, Address senderAddress,Address destinationAdress, String serialOfCustomer) {
		packageID = countID++;
		this.priority=priority;
		this.status=Status.CREATION;
		this.senderAddress=senderAddress;
		this.destinationAddress=destinationAdress;
		this.serialOfCustomer = serialOfCustomer; // my code
		tracking.add(new Tracking( MainOffice.getClock(), null, status, this.packageID));
		readWriteFile.writeToFile(this);
	}	
	...
	{
		tracking.add(new Tracking(MainOffice.getClock(), node, status, this.packageID));
		this.readWriteFile.writeToFile(this); // my code
	}
